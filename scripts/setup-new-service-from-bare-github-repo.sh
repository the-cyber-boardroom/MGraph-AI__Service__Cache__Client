...
git fetch template
git merge template/main --allow-unrelated-histories -m "Initial import from MGraph-AI__Service__Cache__Client template"
print_success "Template merged"

# Step 5: Rename the service directory
print_step "Renaming service directory from mgraph_ai_service_cache__client to $SERVICE_NAME_UNDERSCORES..."
if [ -d "mgraph_ai_service_cache__client" ]; then
    mv mgraph_ai_service_cache__client "$SERVICE_NAME_UNDERSCORES"
    print_success "Service directory renamed"
else
    print_error "mgraph_ai_service_cache__client directory not found!"
    exit 1
fi
...
# Replace "MGraph AI Service Cache Client" with the new service title
replace_in_files "MGraph AI Service Cache Client" "$SERVICE_NAME_TITLE" "service title"

# Replace "mgraph_ai_service_cache__client" with the new package name
replace_in_files "mgraph_ai_service_cache__client" "$SERVICE_NAME_UNDERSCORES" "Python package name"

# Replace "MGraph-AI__Service__Cache__Client" with the new repository name
replace_in_files "MGraph-AI__Service__Cache__Client" "$REPO_NAME" "repository name"
...
- Based on MGraph-AI__Service__Cache__Client v0.3.0
