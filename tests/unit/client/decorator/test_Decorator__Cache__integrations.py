...
# todo see if we have repeated tests with the other decorator_cache test classes
class test_Decorator__Cache__integrations(TestCase):                                                              # Test Decorator__Cache helper class using actual in-memory cache service
    ...
    def setUpClass(cls):                                                                            # Set up in-memory cache service for all tests
        ...
        # cls.cache_service_client = Client__Cache__Service(config=cls.client_config)                     # Create cache client
        cls.cache_service_client = register_cache_service__in_memory(return_client=True)
        cls.decorator_cache      = Decorator__Cache(cache_service_client = cls.cache_service_client)        # Create Decorator__Cache instance
    ...
    def test_operations__lazy_loading(self):                                                        # Test that operations is created on first access
        decorator = Decorator__Cache(cache_service_client = self.cache_service_client)              # Create fresh instance
    ...
    def test_is_available__without_client(self):                                                    # Test availability check when no client is configured
        decorator_no_client = Decorator__Cache(cache_service_client = None)
    ...
    def test_status_with_exception(self):                                                           # Test status handling when operations raise exceptions
        decorator = Decorator__Cache(cache_service_client = self.cache_service_client)
