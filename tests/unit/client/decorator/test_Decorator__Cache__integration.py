...
class test_Decorator__Cache__integration(TestCase):             # Integration tests for Cache__Decorator using real in-memory cache service

    @classmethod
    def setUpClass(cls):
        cls.cache_service_client = register_cache_service__in_memory(return_client=True)
        cls.decorator_cache      = Decorator__Cache(cache_service_client = cls.cache_service_client)          # Create Decorator__Cache instance for services
