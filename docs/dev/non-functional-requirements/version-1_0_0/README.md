# Creating Services from MGraph-AI__Service__Cache__Client
...
curl -O https://raw.githubusercontent.com/the-cyber-boardroom/MGraph-AI__Service__Cache__Client/main/setup-from-template.sh
chmod +x setup-from-template.sh
./setup-from-template.sh
```

The script will:
1. Pull the template from `MGraph-AI__Service__Cache__Client`
...
git remote add template https://github.com/the-cyber-boardroom/MGraph-AI__Service__Cache__Client.git
git fetch template
...
# 4. Commit changes
git add .
git commit -m "Initialize from MGraph-AI__Service__Cache__Client"
...
# Rename directories
find . -type d -name "*mgraph_ai_service_cache__client*" | while read dir; do
    newdir=$(echo "$dir" | sed "s/mgraph_ai_service_cache__client/$SERVICE_NAME/g")
    mv "$dir" "$newdir"
done
...
find . -type f \( -name "*.py" -o -name "*.md" -o -name "*.yml" -o -name "*.yaml" -o -name "*.toml" -o -name "*.sh" -o -name "*.txt" \) | while read file; do
    ...
    sed -i.bak "s/mgraph_ai_service_cache__client/$SERVICE_NAME/g" "$file"        
    sed -i.bak "s/MGraph-AI__Service__Cache__Client/$REPO_NAME/g" "$file"
...
- Based on MGraph-AI__Service__Cache__Client v1.0.0
...
| `MGraph-AI__Service__Cache__Client` | `mgraph_ai_service_cache__client` | MGraph AI Service Cache Client |
| `MGraph-AI__Service__Data-Pipeline` | `mgraph_ai_service_data_pipeline` | MGraph-AI Service Data Pipeline |

## 📁 Template Structure

The template uses `mgraph_ai_service_cache__client` as the placeholder service name:

```
MGraph-AI__Service__Cache__Client/
├── mgraph_ai_service_cache__client/     # Will be renamed to your service
├── tests/
├── docs/
├── scripts/
...
- Always use `mgraph_ai_service_cache__client` as the placeholder
